//+------------------------------------------------------------------+
//|  ARX_AI_Universal_Agent_v152.mq5                                 |
...
//|                                                                  |
//|  v1.52 (Task #473) adds the LIVE broker market-data stream: on a |
//|  fast cadence the EA POSTs the just-closed CANDLE (and, opt-in,  |
//|  throttled TICK) per subscribed symbol+timeframe to the v2       |
//|  ingest path POST /api/bridge/v2/ingest using the per-user       |
//|  X-MT5-Bridge-Token. Each message carries the full v2 envelope   |
//|  (protocolVersion=2, messageType, streamKey, monotonic per-stream|
//|  sequence, idempotencyKey, eaCreatedAtEpochMs, eaVersion) so the |
//|  domain validateBridgeV2Message() passes on first contact and    |
//|  the server feeds fresh bars into the in-memory mt5Provider —     |
//|  making chart/scanner reads broker-native in real time. This is  |
//|  read-only telemetry: it touches no order, gate, arx_live_*       |
//|  table, balance, or fill (separate from the v1.51 durable        |
//|  /api/mt5/candles/ingest history producer, which is preserved).  |
//|  A bar is pushed exactly once (per-stream last-open-time guard);  |
//|  only CLOSED bars are sent; an in-progress bar is never marked    |
//|  closed; stale/duplicate pushes are dropped server-side.         |
//|                                                                  |
//|  ─────────────────────────────────────────────────────────────  |
...
#property strict
#property description "ARX AI Universal MT5 Agent v1.52 — universal command engine, symbol discovery, structured errors, validate-only, broker candle-history producer + live v2 broker market-data stream (safe defaults)."
...
//─── Broker candle-history producer (Task #471) ────────────────────
// Read-only market-data telemetry. The EA POSTs CLOSED-bar batches per
// symbol+timeframe to /api/mt5/candles/ingest and then follows the
// server's nextBackfillHints (suggestedEndTimeUtc) to page OLDER history
// until the series reports COMPLETE or BROKER_LIMITED. No order, no gate,
// no execution side effect — purely chart/scanner candle supply.
input bool    StreamCandleHistory          = true;   // master switch for the producer
input string  CandleSymbols                = "";     // CSV (e.g. "EURUSD,XAUUSD"); empty = chart symbol
input string  CandleTimeframes             = "M1,M5,M15,H1,H4,D1"; // pinned server enum
input int     CandlePushIntervalSeconds    = 30;     // closed-bar push cadence
input int     CandleLiveBatchBars          = 200;    // recent closed bars per live push
input int     CandleBackfillBatchBars      = 1000;   // bars per backfill page (<=5000)
input int     CandleBackfillPagesPerCycle  = 3;      // older pages walked per series each cycle
input int     CandleMaxSymbols             = 40;     // hard cap on parsed CandleSymbols

//─── Live broker market-data stream (Task #473) ────────────────────
// Read-only telemetry over the v2 ingest path POST /api/bridge/v2/ingest.
// On a fast cadence the EA emits the just-closed CANDLE (and, opt-in,
// throttled TICK) per subscribed symbol+timeframe so the server feeds the
// in-memory mt5Provider and the chart/scanner become broker-native in real
// time. SEPARATE from the durable candle-history producer above. No order,
// no gate, no arx_live_* write — purely live market-data supply. Re-uses the
// same CandleSymbols / CandleTimeframes / CandleMaxSymbols subscription set.
input bool    StreamLiveBrokerData         = true;   // master switch for the live v2 stream
input int     LiveStreamIntervalSeconds    = 2;      // sweep cadence (closed-bar + tick check)
input bool    EnableLiveTickStream         = true;   // also push throttled TICK quotes
input int     LiveTickThrottleMs           = 1000;   // min ms between TICK pushes per symbol
...
datetime      lastCandlePushAt  = 0;
datetime      lastCommandAt     = 0;
string        lastSuccessAction = "";
string        lastErrorSeen     = "";
string        EA_VERSION        = "1.52";
string        EA_NAME           = "ARX_AI_Universal_Agent";
string        EA_BUILD          = "1520";
string        BRIDGE_VERSION    = "2";   // command-protocol version

//─── Live v2 broker-stream state (Task #473) ───────────────────────
// Read-only telemetry. Per-stream monotonic sequence + last-pushed bar
// open-time registries (parallel arrays keyed by "<messageType>#<streamKey>"
// and "<symbol>|<tf>") guarantee a CLOSED bar is emitted exactly once and the
// server's gap/duplicate/reset detector stays satisfied. instanceId scopes the
// sequence epoch — it is regenerated on every (re)load so the server records an
// honest RESET (sequence restarts at 0) rather than a fabricated gap.
// Capacity is sized to the CLAMPED worst case so the per-stream sequence
// registry can NEVER overflow under any supported config: every subscribed
// symbol contributes up to V2_MAX_LIVE_TFS candle streams + 1 tick stream.
// PushLiveBrokerStream clamps symCount/tfCount to these bounds (and logs once
// if a config exceeds them), so v2SeqKeys/v2BarKeys/v2TickKeys always fit.
#define V2_MAX_LIVE_SYMBOLS 60
#define V2_MAX_LIVE_TFS     16
#define V2_STREAM_CAPACITY  (V2_MAX_LIVE_SYMBOLS * (V2_MAX_LIVE_TFS + 1)) // 1020 candle+tick keys
string        v2InstanceId      = "";
long          v2IdemCounter     = 0;
datetime      lastLiveStreamAt  = 0;
bool          v2SymbolClampWarned = false; // one-shot log when config exceeds V2_MAX_LIVE_SYMBOLS

string        v2SeqKeys[V2_STREAM_CAPACITY];   // "<messageType>#<streamKey>"
long          v2SeqNext[V2_STREAM_CAPACITY];   // next sequence to emit
int           v2SeqCount        = 0;

string        v2BarKeys[V2_STREAM_CAPACITY];   // "<symbol>|<tf>"
datetime      v2BarLastOpen[V2_STREAM_CAPACITY]; // last CLOSED bar open-time pushed
int           v2BarCount        = 0;

string        v2TickKeys[V2_STREAM_CAPACITY];  // "<symbol>"
ulong         v2TickLastAt[V2_STREAM_CAPACITY]; // last TICK push (GetTickCount64 ms)
int           v2TickCount       = 0;
...
int OnInit()
{
   ...
   if(StringLen(BridgeBaseUrl) == 0 || StringLen(BridgeToken) == 0)
      PrintFormat("ARX Universal Agent v%s — BridgeBaseUrl or BridgeToken missing; EA will idle.", EA_VERSION);
   // Per-(re)load instance id. Scopes the v2 idempotency key namespace and makes
   // a restart an HONEST sequence RESET (server sees seq 0 again), never a gap.
   v2InstanceId = (string)AccountInfoInteger(ACCOUNT_LOGIN) + "-" + (string)(long)TimeGMT() + "-" + (string)GetTickCount();
   EventSetTimer(1);
...
void OnTimer()
{
   ...
   if(StreamCandleHistory && now - lastCandlePushAt >= CandlePushIntervalSeconds)
      { PushCandleHistory(); lastCandlePushAt = now; }
   if(StreamLiveBrokerData && now - lastLiveStreamAt >= LiveStreamIntervalSeconds)
      { PushLiveBrokerStream(); lastLiveStreamAt = now; }
}
...
string BuildCapabilitiesJson()
{
   ...
   // v2 market-data telemetry advisory flags (Task #471 history + #473 live).
   // Read-only — these never advertise an execution capability.
   s += "\"candleHistoryProducer\":" + (StreamCandleHistory ? "true" : "false") + ",";
   s += "\"liveBrokerStream\":"       + (StreamLiveBrokerData ? "true" : "false") + ",";
   s += "\"liveCandleStream\":"       + (StreamLiveBrokerData ? "true" : "false") + ",";
   s += "\"liveTickStream\":"         + ((StreamLiveBrokerData && EnableLiveTickStream) ? "true" : "false") + ",";
   // v2-contract canonical capability keys for the live broker market-data
   // stream (POST /api/bridge/v2/ingest). Mirror the live*Stream flags so the
   // EA advertises the same vocabulary the v2 Beta Kernel contract documents.
   s += "\"candleStream\":"           + (StreamLiveBrokerData ? "true" : "false") + ",";
   s += "\"tickStream\":"             + ((StreamLiveBrokerData && EnableLiveTickStream) ? "true" : "false") + ",";
...
//|  BROKER CANDLE-HISTORY PRODUCER (Task #471)                       |
//|                                                                  |
//|  Streams CLOSED bars per subscribed symbol+timeframe to          |
//|  /api/mt5/candles/ingest and then follows the server's           |
//|  nextBackfillHints (suggestedEndTimeUtc) to page OLDER history    |
//|  until the series reports COMPLETE or BROKER_LIMITED. This is     |
//|  read-only market-data telemetry: no order, no gate, no DB        |
//|  execution write. Bar OPEN times are sent as epoch milliseconds   |
//|  ((long)rate.time * 1000) — the same convention the v2 kernel     |
//|  uses — so the round-trip with the server cursor is consistent.   |
//+------------------------------------------------------------------+
...
int CandleSplitCsv(const string csv, string &out[], int cap)
{
   int count = 0, start = 0, n = StringLen(csv);
   for(int i = 0; i <= n; i++) {
      if(i == n || StringGetCharacter(csv, i) == ',') {
         string tok = TrimStr(StringSubstr(csv, start, i - start));
         if(StringLen(tok) > 0 && count < cap) {
...
ENUM_TIMEFRAMES CandleTfEnum(const string name)
{
   ...
   return (ENUM_TIMEFRAMES)-1; // unsupported (server rejects too)
}

int CandleTfSeconds(const string name)
{
   string u = name; StringToUpper(u);
   if(u == "M1")  return 60;
   if(u == "M5")  return 300;
   if(u == "M15") return 900;
   if(u == "H1")  return 3600;
   if(u == "H4")  return 14400;
   if(u == "D1")  return 86400;
   return 0;
}

// "2026-06-07T08:00:00.000Z" -> datetime (numeric epoch space matches rate.time)
datetime CandleIsoToTime(const string iso)
{
...
// POST a body and RETURN the response (so we can read nextBackfillHints).
string PostJsonGetBody(const string path, const string json)
{
   char post[]; char result[]; string respHeaders;
   string url = BridgeBaseUrl + path;
   int len = StringToCharArray(json, post, 0, WHOLE_ARRAY, CP_UTF8);
   if(len > 0) ArrayResize(post, len - 1); // drop the trailing NUL only
   string reqHeaders = "X-MT5-Bridge-Token: " + BridgeToken
                     + "\r\nContent-Type: application/json\r\nAccept: application/json\r\n";
   ResetLastError();
   int code = WebRequest("POST", url, reqHeaders, 10000, post, result, respHeaders);
   if(code != 200) {
      PrintFormat("ARX Universal Agent POST %s -> %d (err=%d)", path, code, GetLastError());
      return "";
   }
   return CharArrayToString(result);
}

string BuildCandleBatchBody(const string sym, const string tfName,
                            const MqlRates &r[], int got, bool brokerLimited)
{
   int digits = (int)SymbolInfoInteger(sym, SYMBOL_DIGITS);
   if(digits <= 0 || digits > 8) digits = 5;

   string bars = "[";
   int written = 0;
   for(int i = 0; i < got; i++) {
      if(r[i].open <= 0 || r[i].high <= 0 || r[i].low <= 0 || r[i].close <= 0) continue;
      if(written > 0) bars += ",";
      bars += "{";
      bars += "\"openTime\":"   + (string)((long)r[i].time * 1000) + ",";
      bars += "\"open\":"       + SafeF(r[i].open,  digits) + ",";
      bars += "\"high\":"       + SafeF(r[i].high,  digits) + ",";
      bars += "\"low\":"        + SafeF(r[i].low,   digits) + ",";
      bars += "\"close\":"      + SafeF(r[i].close, digits) + ",";
      bars += "\"tickVolume\":" + (string)(long)r[i].tick_volume + ",";
      bars += "\"isClosed\":true";
      bars += "}";
      written++;
   }
   ...
   string body = "{";
   body += "\"symbol\":\""        + StringSubstr(sym, 0, 32) + "\",";
   body += "\"brokerSymbol\":\""  + StringSubstr(sym, 0, 32) + "\",";
   body += "\"timeframe\":\""     + tfName + "\",";
   body += "\"eaVersion\":\""     + EA_VERSION + "\",";
   body += "\"accountNumber\":\"" + (string)AccountInfoInteger(ACCOUNT_LOGIN) + "\",";
   body += "\"brokerServerTime\":"+ (string)((long)TimeCurrent() * 1000) + ",";
   body += "\"sentAt\":"          + (string)((long)TimeGMT() * 1000) + ",";
   body += "\"brokerLimited\":"   + (brokerLimited ? "true" : "false") + ",";
   body += "\"bars\":"            + bars;
   body += "}";
   return body;
}
...
//+------------------------------------------------------------------+
//|  Stream one symbol+timeframe: live closed bars, then walk OLDER. |
//+------------------------------------------------------------------+
void ProcessCandleSeries(const string sym, const string tfNameIn)
{
   string tfName = tfNameIn; StringToUpper(tfName);
   ENUM_TIMEFRAMES tf = CandleTfEnum(tfName);
   int tfSec = CandleTfSeconds(tfName);
   if(tfSec == 0) return; // unsupported timeframe — skip (server pins the enum)

   // ── 1) Live push: most recent CLOSED bars (index 1 skips the forming bar) ──
   int liveCount = CandleLiveBatchBars;
   if(liveCount < 1)    liveCount = 1;
   if(liveCount > 5000) liveCount = 5000;
   MqlRates live[];
   int gotLive = CopyRates(sym, tf, 1, liveCount, live);
   if(gotLive <= 0) return; // history not ready yet — retry next cycle

   string body = BuildCandleBatchBody(sym, tfName, live, gotLive, false);
   string resp = PostJsonGetBody("/api/mt5/candles/ingest", body);
   if(StringLen(resp) == 0) return;

   string status  = JsonStr(resp, "status");             // hint.status
   string suggEnd = JsonStr(resp, "suggestedEndTimeUtc");

   // ── 2) Backfill: follow the server hint OLDER until COMPLETE/BROKER_LIMITED ─
   int pageBars = CandleBackfillBatchBars;
   if(pageBars < 1)    pageBars = 1;
   if(pageBars > 5000) pageBars = 5000;

   int pages = 0;
   while(pages < CandleBackfillPagesPerCycle) {
      if(status == "COMPLETE" || status == "BROKER_LIMITED") break;
      if(suggEnd == "" || suggEnd == "null") break;
      datetime toT = CandleIsoToTime(suggEnd);
      if(toT <= 0) break;
      datetime fromT = toT - (datetime)((long)pageBars * (long)tfSec);

      MqlRates page[];
      int gotPage = CopyRates(sym, tf, fromT, toT, page);
      if(gotPage <= 0) break; // nothing in this window (or still loading) — retry later

      // Honest BROKER_LIMITED signal: the broker's first available bar is at or
      // after this page's oldest bar, so there is nothing older to fetch.
      datetime firstAvail = (datetime)SeriesInfoInteger(sym, tf, SERIES_FIRSTDATE);
      datetime oldestBar  = page[0].time;
      for(int i = 1; i < gotPage; i++) if(page[i].time < oldestBar) oldestBar = page[i].time;
      bool limited = (firstAvail > 0 && oldestBar <= firstAvail);

      string pbody = BuildCandleBatchBody(sym, tfName, page, gotPage, limited);
      string presp = PostJsonGetBody("/api/mt5/candles/ingest", pbody);
      if(StringLen(presp) == 0) break;

      status = JsonStr(presp, "status");
      string newSugg = JsonStr(presp, "suggestedEndTimeUtc");
      if(limited) break; // server now records BROKER_LIMITED; stop this series

      // Cursor must move OLDER each page; otherwise stop to avoid a tight loop.
      if(newSugg == "" || newSugg == "null" || newSugg == suggEnd) break;
      suggEnd = newSugg;
      pages++;
   }
}
...
//+------------------------------------------------------------------+
//|  Producer entry: iterate the configured symbols x timeframes.    |
//+------------------------------------------------------------------+
void PushCandleHistory()
{
   if(StringLen(BridgeBaseUrl) == 0 || StringLen(BridgeToken) == 0) return;

   string syms[];
   int symCount = 0;
   if(StringLen(TrimStr(CandleSymbols)) == 0) {
      ArrayResize(syms, 1); syms[0] = _Symbol; symCount = 1;   // default: chart symbol
   } else {
      int cap = (CandleMaxSymbols > 0 ? CandleMaxSymbols : 1);
      symCount = CandleSplitCsv(CandleSymbols, syms, cap);
   }

   string tfs[];
   int tfCount = CandleSplitCsv(CandleTimeframes, tfs, 16);
   if(tfCount == 0) { ArrayResize(tfs, 1); tfs[0] = "M5"; tfCount = 1; }

   for(int si = 0; si < symCount; si++) {
      string sym = syms[si];
      if(!SymbolSelect(sym, true)) continue; // make the series reachable (Market Watch)
      for(int ti = 0; ti < tfCount; ti++)
         ProcessCandleSeries(sym, tfs[ti]);
   }
}

//+------------------------------------------------------------------+
//|  LIVE v2 BROKER MARKET-DATA STREAM (Task #473)                   |
//|                                                                  |
//|  Emits the just-closed CANDLE (index 1) and, opt-in, a throttled |
//|  TICK per subscribed symbol+timeframe over POST /api/bridge/v2/  |
//|  ingest. Read-only telemetry: no order, no gate, no arx_live_*    |
//|  write. A bar is pushed EXACTLY ONCE via the per-stream last-open |
//|  guard; only CLOSED bars are sent (isClosed:true is never set on  |
//|  a forming bar). SEPARATE from the durable candle-history         |
//|  producer above (that POSTs batches to /api/mt5/candles/ingest).  |
//+------------------------------------------------------------------+
long V2NowEpochMs()
{
   long s = (long)TimeGMT();
...
// returns 0 (server records FIRST), then 1, 2, … (IN_ORDER). v2InstanceId is
// regenerated on every (re)load, so a restart is an honest RESET, not a gap.
long V2NextSequence(const string messageType, const string streamKey)
{
   string key = messageType + "#" + streamKey;
   for(int i = 0; i < v2SeqCount; i++) {
      if(v2SeqKeys[i] == key) { long v = v2SeqNext[i]; v2SeqNext[i] = v + 1; return v; }
   }
   if(v2SeqCount < V2_STREAM_CAPACITY) {
      v2SeqKeys[v2SeqCount] = key; v2SeqNext[v2SeqCount] = 1; v2SeqCount++; return 0;
   }
   // Registry full (cannot happen with the clamp below, kept as a hard guard):
   // return -1 so PostV2Ingest DROPS the message rather than emitting a
   // non-contiguous sequence that the server would treat as a gap/reset.
   return -1;
}

// Globally-unique-per-connection idempotency key (>= 8 chars, <= 128).
string V2NextIdemKey()
{
   v2IdemCounter++;
   return v2InstanceId + "-" + (string)v2IdemCounter;
}

// POST one fully-formed v2 envelope. One idempotent retry on transport failure
// reuses the SAME sequence + idempotencyKey so the server dedupes (never
// double-records). Returns true only on HTTP 200.
bool PostV2Ingest(const string messageType, const string streamKey, const string payloadJson)
{
   long   seq   = V2NextSequence(messageType, streamKey);
   if(seq < 0) return false; // sequence registry exhausted — drop, never corrupt ordering
   string idem  = V2NextIdemKey();
   long   nowMs = V2NowEpochMs();
   ...
   char post[]; char result[]; string respHeaders;
   string url = BridgeBaseUrl + "/api/bridge/v2/ingest";
   int len = StringToCharArray(env, post, 0, WHOLE_ARRAY, CP_UTF8);
   if(len > 0) ArrayResize(post, len - 1); // drop the trailing NUL only
   string reqHeaders = "X-MT5-Bridge-Token: " + BridgeToken
                     + "\r\nContent-Type: application/json\r\nAccept: application/json\r\n";
   ResetLastError();
   ...
   if(code != 200) {
      ResetLastError();
      code = WebRequest("POST", url, reqHeaders, 10000, post, result, respHeaders); // idempotent retry
   }
   if(code != 200) {
      PrintFormat("ARX v2 ingest %s/%s -> %d (err=%d)", messageType, streamKey, code, GetLastError());
      return false;
   }
   return true;
}

// Push the latest CLOSED bar for symbol+tf — exactly once (last-open guard).
void PushLiveCandleV2(const string sym, const string tfName, ENUM_TIMEFRAMES tf)
{
   MqlRates r[];
   if(CopyRates(sym, tf, 1, 1, r) != 1) return;   // index 1 = last CLOSED bar
   if(r[0].open <= 0 || r[0].high <= 0 || r[0].low <= 0 || r[0].close <= 0) return;

   string barKey = sym + "|" + tfName;
   for(int i = 0; i < v2BarCount; i++)
      if(v2BarKeys[i] == barKey) { if(v2BarLastOpen[i] == r[0].time) return; break; }
   ...
   payload += "\"open\":"  + SafeF(r[0].open,  digits) + ",";
   payload += "\"high\":"  + SafeF(r[0].high,  digits) + ",";
   payload += "\"low\":"   + SafeF(r[0].low,   digits) + ",";
   payload += "\"close\":" + SafeF(r[0].close, digits) + ",";
   payload += "\"volume\":"+ (string)(long)r[0].tick_volume + ",";
   payload += "\"isClosed\":true";
   payload += "}";

   if(!PostV2Ingest("CANDLE", StringSubstr(sym, 0, 32) + "|" + tfName, payload)) return;

   // Record the open-time ONLY after an accepted push, so a failed send retries.
   for(int i = 0; i < v2BarCount; i++)
      if(v2BarKeys[i] == barKey) { v2BarLastOpen[i] = r[0].time; return; }
   if(v2BarCount < V2_STREAM_CAPACITY) { v2BarKeys[v2BarCount] = barKey; v2BarLastOpen[v2BarCount] = r[0].time; v2BarCount++; }
}

// Push a throttled latest-quote TICK for symbol.
void PushLiveTickV2(const string sym)
{
   ulong nowMs    = GetTickCount64();
   ulong throttle = (ulong)(LiveTickThrottleMs < 0 ? 0 : LiveTickThrottleMs);
   for(int i = 0; i < v2TickCount; i++)
      if(v2TickKeys[i] == sym) { if(nowMs - v2TickLastAt[i] < throttle) return; break; }
   ...
   if(tEpochMs <= 0) tEpochMs = V2NowEpochMs();
   ...
   string payload = "{";
   payload += "\"symbol\":\""          + StringSubstr(sym, 0, 32) + "\",";
   payload += "\"bid\":"               + SafeF(t.bid, digits) + ",";
   payload += "\"ask\":"               + SafeF(t.ask, digits) + ",";
   payload += "\"brokerTimeEpochMs\":" + (string)tEpochMs;
   payload += "}";

   if(!PostV2Ingest("TICK", StringSubstr(sym, 0, 32), payload)) return;

   for(int i = 0; i < v2TickCount; i++)
      if(v2TickKeys[i] == sym) { v2TickLastAt[i] = nowMs; return; }
   if(v2TickCount < V2_STREAM_CAPACITY) { v2TickKeys[v2TickCount] = sym; v2TickLastAt[v2TickCount] = nowMs; v2TickCount++; }
}

//+------------------------------------------------------------------+
//|  Sweep entry: iterate the configured symbols x timeframes.       |
//|  Re-uses the candle-history subscription set so ONE config drives |
//|  both the durable history producer and the live stream.          |
//+------------------------------------------------------------------+
void PushLiveBrokerStream()
{
   if(StringLen(BridgeBaseUrl) == 0 || StringLen(BridgeToken) == 0) return;
   ...
   string tfs[];
   int tfCount = CandleSplitCsv(CandleTimeframes, tfs, V2_MAX_LIVE_TFS);
   if(tfCount == 0) { ArrayResize(tfs, 1); tfs[0] = "M5"; tfCount = 1; }

   // Clamp to the registry-capacity bound so the per-stream sequence registry
   // can never overflow (each symbol = up to tfCount candle streams + 1 tick).
   // Honest: if a config exceeds the bound the excess symbols are skipped and a
   // one-shot warning is logged — we never silently corrupt sequence ordering.
   if(symCount > V2_MAX_LIVE_SYMBOLS) {
      if(!v2SymbolClampWarned) {
         PrintFormat("ARX v2 live-stream: %d symbols exceeds V2_MAX_LIVE_SYMBOLS=%d; streaming the first %d only.",
                     symCount, V2_MAX_LIVE_SYMBOLS, V2_MAX_LIVE_SYMBOLS);
         v2SymbolClampWarned = true;
      }
      symCount = V2_MAX_LIVE_SYMBOLS;
   }

   for(int si = 0; si < symCount; si++) {
      string sym = syms[si];
      if(!SymbolSelect(sym, true)) continue;     // make the series reachable (Market Watch)
      if(EnableLiveTickStream) PushLiveTickV2(sym);
      for(int ti = 0; ti < tfCount; ti++) {
         string tfName = tfs[ti]; StringToUpper(tfName);
         if(CandleTfSeconds(tfName) == 0) continue;  // unsupported tf — server pins the enum
         PushLiveCandleV2(sym, tfName, CandleTfEnum(tfName));
      }
   }
}
//+------------------------------------------------------------------+
