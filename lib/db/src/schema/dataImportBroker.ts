...
import { pgTable, serial, text, integer, real, jsonb, boolean, timestamp, uniqueIndex } from "drizzle-orm/pg-core";
...
export const brokerReadonlySnapshotsTable = pgTable("broker_readonly_snapshots", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => ({ snapshotIdIdx: uniqueIndex("broker_readonly_snapshots_snapshot_id_idx").on(t.snapshotId) }));
