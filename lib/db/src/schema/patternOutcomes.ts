// Task #617 — Chart Pattern Truth learning loop.
//
// SAFETY / SCOPE:
//   - OBSERVATION ONLY. A row records a chart pattern that the deterministic
//     detector already surfaced, plus what happened AFTER. Writing/resolving a
//     row NEVER places, modifies or closes a trade and NEVER touches the MT5
//     bridge or the live (16-gate) pipeline. It is read-only over outcomes.
//   - The reliability stats derived from these rows feed ONLY Ruby's bounded
//     confidence ADJUSTMENT — they remain subject to the Pattern Truth hard
...
// shared PatternTruthVerdict contract:
//   bias              : bullish | bearish | neutral
//   statusAtDetection : forming | confirmed | failed | exhausted | invalidated
...
export const patternOutcomesTable = pgTable(
  "pattern_outcomes",
  {
    id: serial("id").primaryKey(),
    // Stable external id for the detected-pattern observation (dedupe key base).
    ...
    patternId: text("pattern_id").notNull(), // e.g. head_and_shoulders
    patternName: text("pattern_name").notNull(),
    patternCategory: text("pattern_category"), // reversal | continuation | ...
    ...
    // Pattern geometry levels at detection.
    ...
    // Optional link to a real trade once one is taken on this pattern.
  ...
  (t) => ({
    userOutcomeUq: uniqueIndex("pattern_outcomes_user_outcome_uq").on(t.userId, t.outcomeId),
    userSymbolTfIdx: index("pattern_outcomes_user_symbol_tf_idx").on(
      t.userId,
      t.symbol,
      t.timeframe,
    ),
    userPatternIdx: index("pattern_outcomes_user_pattern_idx").on(t.userId, t.patternId),
    userOutcomeStatusIdx: index("pattern_outcomes_user_outcome_status_idx").on(
      t.userId,
      t.outcome,
    ),
  }),
...
export type PatternOutcome = typeof patternOutcomesTable.$inferSelect;
export type NewPatternOutcome = typeof patternOutcomesTable.$inferInsert;
