// ── TRENDLINE RELIABILITY — PURE AGGREGATION (Task #649, Phase 7) ─────────────
//
// PURE, deterministic aggregation of recorded trendline OUTCOMES into reliability
...
//
// ── SAFETY: still subject to the Trendline Truth hard boundary ────────────────
...
/** One resolved (or pending) trendline outcome, the minimal shape aggregation needs. */
export interface TrendlineOutcomeSample {
  symbol: string;
  timeframe: string;
  ...
  outcome: TrendlineOutcomeGrade;
  realizedR: number | null;
  mfeR: number | null;
  maeR: number | null;
}

export type TrendlineOutcomeGrade =
  | "PENDING"
  | "WIN"
  | "LOSS"
  | "BREAKEVEN"
...
const RESOLVED_GRADES: ReadonlySet<TrendlineOutcomeGrade> = new Set([
  "WIN",
  "LOSS",
  "BREAKEVEN",
  "FALSE_POSITIVE",
...
const WIN_GRADES: ReadonlySet<TrendlineOutcomeGrade> = new Set(["WIN"]);

/** Outcomes counted as a false positive (confirmed look, immediately failed). */
const FALSE_POSITIVE_GRADES: ReadonlySet<TrendlineOutcomeGrade> = new Set([
  "FALSE_POSITIVE",
  "INVALIDATED",
]);

export interface TrendlineReliabilityBucket {
  /** Group key — symbol, timeframe, session, or trendline for the relevant report. */
  key: string;
...
export interface TrendlineReliabilityReport {
  /** "forex_indices" (forex + indices) or "synthetic" — tracked separately. */
  market: TrendlineMarketClass;
  totalSamples: number;
  resolvedCount: number;
  overall: TrendlineReliabilityBucket;
  bySymbol: TrendlineReliabilityBucket[];
  byTimeframe: TrendlineReliabilityBucket[];
  bySession: TrendlineReliabilityBucket[];
  byTrendline: TrendlineReliabilityBucket[];
...
export type TrendlineMarketClass = "forex_indices" | "synthetic";
 ...
 */
export const TRENDLINE_MAX_CONFIDENCE_ADJUSTMENT = 8;

/** Minimum resolved samples before a reliability score / adjustment is trusted. */
export const TRENDLINE_MIN_RESOLVED_FOR_SCORE = 5;

function isResolved(g: TrendlineOutcomeGrade): boolean {
...
function buildBucket(key: string, samples: TrendlineOutcomeSample[]): TrendlineReliabilityBucket {
  ...
  return {
    ...
    avgRealizedR: round(avg(resolved.map((s) => s.realizedR)), 3),
...
function groupBy(
  samples: TrendlineOutcomeSample[],
  keyFn: (s: TrendlineOutcomeSample) => string | null,
): TrendlineReliabilityBucket[] {
  const map = new Map<string, TrendlineOutcomeSample[]>();
 ...
 * false-positive rate. Returns null until TRENDLINE_MIN_RESOLVED_FOR_SCORE
 * resolved samples — never guesses off a tiny sample.
 */
function reliabilityScore(overall: TrendlineReliabilityBucket): number | null {
  if (overall.resolvedCount < TRENDLINE_MIN_RESOLVED_FOR_SCORE) return null;
 ...
 * neutral score of 50 and clamped to ±TRENDLINE_MAX_CONFIDENCE_ADJUSTMENT.
 * Returns 0 when there is not enough resolved evidence (never a default bonus).
 */
function confidenceAdjustment(score: number | null): number {
  if (score === null) return 0;
  const centred = (score - 50) / 50; // -1..+1
  const adj = centred * TRENDLINE_MAX_CONFIDENCE_ADJUSTMENT;
  const clamped = Math.max(
    -TRENDLINE_MAX_CONFIDENCE_ADJUSTMENT,
    Math.min(TRENDLINE_MAX_CONFIDENCE_ADJUSTMENT, adj),
  );
  return round(clamped, 2)!;
}

/** Best/worst session by win rate among sessions with resolved samples. */
function bestWorstSession(bySession: TrendlineReliabilityBucket[]): {
  best: string | null;
  worst: string | null;
} {
 ...
 */
export function trendlineMarketClass(isSynthetic: boolean): TrendlineMarketClass {
 ...
 * `aggregateTrendlineReliability` for both at once).
 */
export function aggregateTrendlineMarket(
  market: TrendlineMarketClass,
  samples: TrendlineOutcomeSample[],
): TrendlineReliabilityReport {
  ...
  return {
    ...
    bySession,
    byTrendline: groupBy(samples, (s) => s.trendlineId),
    bestSession: best,
    worstSession: worst,
 ...
 */
export function aggregateTrendlineReliability(samples: TrendlineOutcomeSample[]): {
  forexIndices: TrendlineReliabilityReport;
  synthetic: TrendlineReliabilityReport;
} {
  const synthetic: TrendlineOutcomeSample[] = [];
  const forexIndices: TrendlineOutcomeSample[] = [];
  for (const s of samples) {
    (s.isSynthetic ? synthetic : forexIndices).push(s);
  }
  return {
    forexIndices: aggregateTrendlineMarket("forex_indices", forexIndices),
    synthetic: aggregateTrendlineMarket("synthetic", synthetic),
  };
}
