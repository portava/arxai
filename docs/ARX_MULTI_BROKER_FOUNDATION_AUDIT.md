...
- current MT5 manual and AI-assisted live actions use the existing Phase B pipeline and its 23 gates;
...
3. **The 23-gate MT5 contract.** The master switch, user arming, approval, global controls, kill switch, account type, bridge freshness/readiness, EA capability, symbol/lot/loss controls, SL/TP governance, and disclosure remain required for MT5. The environment switch is necessary but never sufficient.
...
| Deterministic risk kernel (§11) | 23-gate Phase B evaluator plus allocation, entitlement and instant-trade guards | Strong but MT5-specific | No new kernel in Phase 1. Before any non-MT5 demo execution, design a broker-neutral deterministic kernel for common gates and venue-specific evidence. MT5 continues through its existing Phase B evaluator unchanged. |
...
| **High** | **Forcing direct-API venues through MT5-specific gates** | The 23-gate evaluator requires EA/terminal evidence that Deriv/OANDA cannot provide. | Preserve Phase B for MT5; before non-MT5 demo, approve a common broker-neutral gate contract plus venue-specific evidence gates. Never satisfy an MT5 gate with fabricated adapter fields. |
...
- preserve the MT5 routing branch exactly as instant trade → Phase B → 23 gates → EA mailbox;
...
The current MT5 bridge, demo lane, Phase B pipeline, 23-gate evaluator, instant-trade router, market-data truth handling, audit evidence, and reconciliation logic are retained and protected for MT5. Non-MT5 execution requires a later broker-neutral deterministic control-plane review; it may not fabricate EA evidence or weaken the common safety outcomes.
