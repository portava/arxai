...
12. **23-gate evaluator** `evaluateLivePhaseBDispatchGate` (1925–1975 → `lib/domain/src/safety-contracts/livePhaseBDispatchGate.ts:117-250`)
...
**Conclusion:** the spec's "deterministic risk kernel" maps to the pipeline + 23-gate stack, NOT to anything named "risk governor". The naming is a trap for future implementers.
...
- **Net:** with routing=USER_OWNED_MT5 (the default), an engaged global emergency stop does not block a live dispatch that satisfies the other 23 gates. Spec check #1 is the FIRST check of the kernel; here it is partially decorative.
...
- The 23-gate evaluator is pure, fixed-order, default-deny, with a truthful per-gate readout (`livePhaseBDispatchGate.ts`) — a real kernel seed.
