...
2. **`trading_workspace_members`** duplicates `user_master_live_access` (+ `beta_invites`, + `user_advanced_permissions.sharedBridgeApproved`). `userAdvancedPermissions.ts:10–18` already had to *legislate* a single source of truth between two tables; a fourth would break the 18-gate evaluator's inputs.
