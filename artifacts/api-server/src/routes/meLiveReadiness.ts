...
//                         dispatch still re-runs the 18 Phase B gates.
//
...
router.get("/me/live-readiness", requireUser, async (req, res) => {
  ...
  if (userBlocked) {
    ...
    execNextStep = "Open Live Arming and complete the manual arming gate. Every dispatch still re-checks all 18 safety gates.";
  } else {
    execHeadline = "Live execution is ready";
    execNextStep = "Place trades from the Live Trade Ticket. All 18 Phase B gates are re-checked per dispatch.";
  }
