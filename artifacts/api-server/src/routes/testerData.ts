...
const router = Router();
const TESTER_TAG = "TESTER_DEMO_SEED";
...
router.post("/tester-data/seed", async (req, res) => {
  try {
    ...
    for (let i = 0; i < 8; i++) {
      const reject = i % 4 === 3;
      intents.push({
        intentId: `intent_seed_${randomUUID()}`,
        source: sources[i % 3]!,
        symbol: symbols[i % 4]!,
    ...
    const journalRows = Array.from({ length: 6 }).map((_, i) => ({
      ...
      strategy: `[${TESTER_TAG}] DEMO_SIMULATOR`,
    ...
    await db.insert(tradeJournalTable).values(journalRows).catch(() => { /* non-fatal */ });

    return res.json({ seeded: true, intents: intents.length, vaultEvents: vaultRows.length, journalEntries: journalRows.length });
  } catch (err) {
...
router.post("/tester-data/clear", async (req, res) => {
  try {
    ...
    const intents = await db.delete(liveIntentsTable)
      .where(sql`${liveIntentsTable.intentId} like 'intent_seed_%'`).returning({ id: liveIntentsTable.id });
    await db.insert(vaultEventsTable).values({
      kind: "TESTER_SEED_CLEARED",
      severity: "INFO",
      source: "TESTER_SEED",
      truthDomain: "SAFETY",
      summary: `Tester demo seed cleared. Seeded vault rows (kind=${TESTER_TAG}) left in place per append-only invariant.`,
      payload: { intentsRemoved: intents.length, vaultRowsRetained: true },
      reasons: ["clear"],
      blockers: [],
      generatedAtIso: new Date().toISOString(),
    });
    return res.json({ cleared: true, intents: intents.length, vaultEventsRetained: true, correctiveEventAppended: true });
  } catch (err) {
