...
export function ControlledLiveTestButton() {
  ...
  return (
    <div className="rounded-lg border border-danger/40 bg-danger/20 p-4 space-y-4">
      <header>
        ...
        <>
          <div className="grid grid-cols-2 gap-2">
            <label className="text-xs text-txt-secondary">
              Side
              <select className="mt-1 w-full rounded bg-card border border-border px-2 py-1 text-sm"
            ...
            <label className="text-xs text-txt-secondary">
              ...
              <input type="number" step="0.00001" value={stopLoss}
                ...
                className="mt-1 w-full rounded bg-card border border-border px-2 py-1 text-sm"
          ...
          <div className="flex gap-2">
            <button type="button" onClick={runPreview} disabled={!canPreview}
              ...
              className={`flex-1 rounded px-3 py-2 text-sm font-semibold ${
                canPreview ? "bg-muted hover:bg-muted text-white" : "bg-secondary text-txt-muted cursor-not-allowed"
              }`}>
            ...
            <button type="button" onClick={confirmStart} disabled={!canConfirm}
              ...
              className={`flex-1 rounded px-3 py-2 text-sm font-semibold ${
                canConfirm ? "bg-danger hover:bg-danger text-white" : "bg-secondary text-txt-muted cursor-not-allowed"
              }`}
          ...
          {confirmDisabledReason && (
            <div className="text-[11px] text-warning" data-testid="ltc-confirm-disabled-reason">
              {confirmDisabledReason}
            </div>
          )}
          ...
          {preview && (
            <div className="rounded bg-card border border-border p-3 text-xs text-foreground space-y-2">
              <div className="font-semibold">
                Preview:{" "}
                <span className={preview.ok ? "text-success" : "text-warning"}>
              ...
              <ul className="space-y-0.5">
                {preview.precheck.map((c) => (
                  <li key={c.key} className="font-mono">
                    ...
                    <span className="text-txt-secondary">{c.key}</span>
                    <span className="text-txt-muted"> — {c.detail}</span>
                  </li>
                ))}
                <li className="font-mono">
                  <span className={preview.preflight.ok ? "text-success" : "text-danger"}>
                    {preview.preflight.ok ? "✓" : "✗"}
                  </span>{" "}
                  <span className="text-txt-secondary">preflight</span>
                  {!preview.preflight.ok && (
                    <span className="text-txt-muted"> — {preview.preflight.reason}{preview.preflight.detail ? `: ${preview.preflight.detail}` : ""}</span>
                  )}
                </li>
              </ul>
              {preview.note && <div className="text-txt-muted italic">{preview.note}</div>}
            </div>
          )}
        </>
      )}

      {err && <div className="text-xs text-danger">Error: {err}</div>}

      {cycle && (
        <section className="rounded bg-card border border-border p-3 text-xs text-foreground space-y-2">
          <div className="flex items-center justify-between">
            <div className="font-semibold">
              Cycle <code className="text-txt-secondary">{cycle.cycleId}</code>
              <span className="ml-2">
                Status:{" "}
                <span className={
                  cycle.status === "COMPLETED" ? "text-success"
                  : cycle.status === "CLOSE_FAILED_MANUAL_REQUIRED" ? "text-danger"
                  : cycle.status.endsWith("REJECTED") || cycle.status === "DRY_RUN_BLOCKED" ? "text-warning"
                  : "text-primary"
                }>
                  {cycle.status}
                </span>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            ...
            <div>
              ...
              {cycle.pnlStatus === "COMPUTED" && typeof cycle.realizedPlUsd === "number" ? (
                ...
                <code className="text-txt-secondary">unavailable</code>
              )}
            </div>
          ...
          {cycle.status === "COMPLETED" && cycle.pnlStatus && cycle.pnlStatus !== "COMPUTED" && (
            <div className="rounded border border-warning/40 bg-warning/20 p-2 text-warning">
              ...
              {eaTooOldForCloseFill(cycle.reportedEaVersion) && (
                ...
                >
                  ...
                  {cycle.reportedEaVersion && (
                    <span className="ml-1 text-warning/70 font-mono">
              ...
              {showAdminDiagnostics && cycle.dataQualityFlag && (
                <div className="mt-1 text-[11px] text-warning/80 font-mono">
          ...
          <div>
            <div className="font-semibold text-txt-secondary">Latency stages</div>
              ...
              <tbody>
                {stageRows.map(([label, when, prev]) => (
                  <tr key={label} className="border-t border-border/60">
                    ...
                    <td className="py-0.5 text-txt-secondary">{fmtTs(when)}</td>
                    <td className="py-0.5 text-right text-txt-secondary">{ms(prev, when)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="text-txt-muted mt-1">
              Open latency: <code className="text-txt-secondary">{openLatency}</code>{" · "}
              Close latency: <code className="text-txt-secondary">{closeLatency}</code>{" · "}
              Total: <code className="text-txt-secondary">{totalLatency}</code>
            </div>
          </div>

          {cycle.openRejectionReason && (
            <div className="text-danger">Open rejection: <code>{cycle.openRejectionReason}</code></div>
          )}
          {cycle.closeRejectionReason && (
            <div className="text-danger">Close rejection: <code>{cycle.closeRejectionReason}</code></div>
          )}
          {cycle.blockReason && (
            <div className="text-warning">Blocked: <code>{cycle.blockGate ?? "?"}</code> — {cycle.blockReason}</div>
          )}
          {cycle.manualResolveNote && (
            <div className="text-txt-secondary">Manual resolve note: {cycle.manualResolveNote}</div>
          )}

          {cycle.status === "CLOSE_FAILED_MANUAL_REQUIRED" && !cycle.manualResolveNote && (
            <div className="space-y-1 border-t border-border pt-2">
              <div className="text-danger font-semibold">Manual close required at broker.</div>
              <input type="text" value={resolveNote} onChange={(e) => setResolveNote(e.target.value)}
                className="w-full rounded bg-card border border-border px-2 py-1 text-xs"
              ...
              <button type="button" onClick={resolveCycle} disabled={busy !== "" || !resolveNote.trim()}
                className="rounded px-3 py-1 text-xs bg-warning/15 hover:bg-warning text-white disabled:opacity-50">
