    ...
    <div className="space-y-5">
      <div>
        <h3 className="text-3xl xl:text-4xl font-black leading-tight">
          Live Market <span className="text-primary">Intelligence</span>
        </h3>
        <p className="mt-3 text-base text-txt-secondary max-w-md">
          See market movement clearly before you act.
        </p>
      </div>

      <div className="relative h-48 overflow-hidden rounded-2xl border border-primary/20 bg-[#06101f]">
        ...
        <div className="absolute bottom-5 left-5 right-5 flex items-end gap-2 h-32">
          {candles.map((h, i) => (
            <div
              key={i}
              className={`flex-1 rounded-sm ${i % 2 === 0 ? "bg-success/80" : "bg-primary/70"} ${animate ? "arx-login-candle" : ""}`}
        ...
        {tickers.map((t) => (
          <div key={t.sym} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
            <div className="flex items-center justify-between">
              ...
              <TrendingUp className={`h-3 w-3 ${t.up ? "text-success" : "text-danger rotate-180"}`} aria-hidden />
            </div>
            <div className="mt-1 flex items-baseline justify-between">
              <span className="text-sm font-mono text-foreground">{t.px}</span>
              <span className={`text-[11px] font-semibold ${t.up ? "text-success" : "text-danger"}`}>{t.chg}</span>
            </div>
          </div>
        ))}
      </div>
...
function SlideRuby({ animate }: { animate: boolean }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-3xl xl:text-4xl font-black leading-tight">
          {name} Trades With <span className="text-primary">Discipline</span>
        </h3>
        <p className="mt-3 text-base text-txt-secondary max-w-md">
      ...
      <div className="space-y-3">
        <div className="flex items-start gap-2.5">
          <div className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-gradient-to-br from-primary to-premium text-xs font-black">R</div>
          <div className="rounded-2xl rounded-tl-sm border border-primary/20 bg-primary/10 px-4 py-2.5 text-sm text-foreground">
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {[
          { label: "Entry", val: "1.1652", color: "text-primary" },
          { label: "Stop Loss", val: "1.1631", color: "text-danger" },
          { label: "Take Profit", val: "1.1694", color: "text-success" },
        ].map((c) => (
          <div key={c.label} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5">
            <p className="text-[10px] uppercase tracking-wider text-txt-secondary">{c.label}</p>
...
function SlideExecution({ animate }: { animate: boolean }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-3xl xl:text-4xl font-black leading-tight">
          From Signal to <span className="text-primary">Execution</span>
        </h3>
        <p className="mt-3 text-base text-txt-secondary max-w-md">
      ...
      <div className="flex items-center justify-between gap-1">
        {steps.map(({ label, Icon }, i) => (
          <div key={label} className="flex items-center gap-1 flex-1">
            ...
            {i < steps.length - 1 && <div className="h-px flex-1 bg-gradient-to-r from-primary/40 to-primary/10 -mt-5" />}
          </div>
        ))}
      </div>

      <div className="rounded-2xl border border-primary/20 bg-black/30 p-4">
        <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
          <span className="text-sm font-bold">EURUSD</span>
          <span className="rounded-md bg-success/15 px-2 py-0.5 text-xs font-semibold text-success">BUY</span>
        </div>
        <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
          <div className="flex justify-between"><span className="text-txt-secondary">Risk / Reward</span><span className="font-mono text-foreground">1 : 2.0</span></div>
          <div className="flex justify-between"><span className="text-txt-secondary">Volume</span><span className="font-mono text-foreground">0.10</span></div>
          <div className="flex justify-between"><span className="text-txt-secondary">Stop Loss</span><span className="font-mono text-danger">1.1631</span></div>
          <div className="flex justify-between"><span className="text-txt-secondary">Take Profit</span><span className="font-mono text-success">1.1694</span></div>
        </div>
        <div className="mt-3 flex items-center gap-2 rounded-lg border border-success/20 bg-success/5 px-3 py-2">
          <CheckCircle2 className="h-4 w-4 text-success" aria-hidden />
          <span className="text-xs text-success">Confirmed — ready when you are.</span>
        </div>
      </div>
    </div>
  );
        ...
        {SLIDES.map((_, i) => (
          <button
            ...
            className={`h-2 rounded-full transition-all ${
              i === idx ? "w-8 bg-primary" : "w-2 bg-white/25 hover:bg-white/40"
            }`}
          />
        ))}
      </div>
