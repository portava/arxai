...
export function RuleViolationFeed({ violations }: { violations: V[] }) {
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-900/40 p-4">
      <h3 className="mb-2 text-sm font-semibold text-slate-100">Violations & warnings ({violations.length})</h3>
      {violations.length === 0
        ? <p className="text-xs text-emerald-400">No violations today — keep going.</p>
        : <ul className="max-h-64 space-y-1 overflow-auto text-xs">
            {violations.map((v, i) => (
              <li key={i} className="flex items-start gap-2 rounded border border-slate-800 bg-slate-950/40 p-2">
                ...
                {v.tradeId != null && <span className="ml-auto font-mono text-[10px] text-slate-500">#{v.tradeId}</span>}
              </li>
            ))}
          </ul>}
    </div>
