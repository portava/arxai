  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={toggle}
        className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-zinc-900/40 rounded-md"
        aria-expanded={open}
        data-testid={testId ? `${testId}-toggle` : undefined}
      >
        <ChevronDown
          className={cn("h-4 w-4 text-zinc-400 transition-transform", open ? "rotate-0" : "-rotate-90")}
        />
