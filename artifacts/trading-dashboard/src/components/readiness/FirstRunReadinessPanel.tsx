...
function StatusIcon({ status }: { status: ReadinessStatus }) {
  ...
  if (status === "FAIL") return <XCircle size={18} className="text-danger shrink-0" />;
  return <Info size={18} className="text-txt-secondary shrink-0" />;
}
    ...
    <Card className="border-card-border" data-testid="first-run-readiness-panel">
      <CardHeader className="pb-3 border-b border-border">
        <CardTitle className="text-sm font-semibold uppercase tracking-wider flex items-center gap-2">
          First-run readiness
          <span className="text-[10px] font-normal text-txt-secondary normal-case">
            live trading armed
          </span>
        </CardTitle>
        {q.data && (
      ...
      <CardContent className="pt-4">
        {q.isLoading && <div className="text-xs text-txt-secondary">Checking your readiness…</div>}
        {q.isError && (
            ...
            {q.data.items.map((it) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-txt-secondary mt-0.5">{it.detail}</div>
                </div>
                {it.cta && (
                  <a
                    href={it.cta.href}
                    className="text-xs text-success hover:text-success inline-flex items-center gap-1 shrink-0"
