...
export function EmotionAfterTradeSelector({ value, onChange }:
  ...
  return (
    <div>
      <div className="mb-1.5 text-xs font-semibold text-foreground">Emotion after the trade</div>
          ...
          return (
            <button key={e.value} type="button" onClick={() => onChange(e.value)}
              className={`flex flex-col items-center rounded border px-1 py-1.5 text-[10px] transition ${
                sel ? "border-ruby bg-ruby/60 text-ruby"
                    : "border-border bg-background/40 text-txt-secondary hover:border-border"}`}>
