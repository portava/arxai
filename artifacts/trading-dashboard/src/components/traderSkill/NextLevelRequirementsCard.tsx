...
export function NextLevelRequirementsCard({ suggestions }: { suggestions: SkillSuggestion[] }) {
  ...
  return (
    <div className="rounded-lg border border-premium/40 bg-premium/20 p-3">
      <h3 className="mb-1 text-sm font-semibold text-premium">Next level requirements</h3>
      {header && <p className="mb-2 text-xs leading-relaxed text-foreground">{header.message}</p>}
      {items.length > 0 && (
        <>
          <p className="mb-1 text-[10px] uppercase tracking-wide text-txt-secondary">Biggest leverage points</p>
            ...
            {items.map((s, i) => (
              <li key={i} className="rounded border border-border bg-background/40 p-2 text-xs">
                <div className="mb-0.5 flex items-center justify-between">
                  ...
                  {s.score !== undefined && <span className="font-mono text-[10px] text-txt-secondary">{s.score}/100</span>}
                </div>
                <p className="text-[11px] text-txt-secondary">{s.message}</p>
              </li>
            ))}
          </ul>
        </>
