...
function HeadlineRow({ h }: { h: NewsHeadline }) {
  return (
    <div className="flex items-start gap-2 rounded-md border border-slate-700 bg-slate-900/40 px-2 py-1.5">
      <span className={`mt-0.5 shrink-0 rounded border px-1 py-0.5 text-[9px] uppercase ${HEADLINE_SEVERITY_STYLE[h.severity]}`}>
        {h.severity}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-xs text-slate-200">{h.headline}</span>
...
const STYLES: Record<Risk, string> = {
  ...
  HIGH_RISK:        "bg-orange-700 text-white",
  NO_TRADE_WINDOW:  "bg-red-700 text-white animate-pulse",
};
  ...
  return (
    <div className="space-y-3 rounded-lg border border-slate-700 bg-slate-900/40 p-4">
      <header className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-slate-100">News & Economic Risk</h3>
      ...
      {providersUnavailable ? (
        <div className="rounded-md border border-slate-600 bg-slate-800/60 p-3 text-xs text-slate-300">
          <span className="font-semibold text-slate-200">Provider unavailable.</span>{" "}
      ...
      {!r && !latest.isLoading && (
        <p className="text-sm text-slate-400">No report yet. Click "Run scan" — sync economic events first via the Calendar page if results look empty.</p>
      )}
      ...
      {r && (
        <>
          <div className="flex flex-wrap items-center gap-3">
            ...
            {r.relatedCurrency && (
              <span className="text-sm text-slate-300">Currency: <span className="font-semibold text-slate-100">{r.relatedCurrency}</span></span>
            )}
          </div>
          {r.tradeWarning && (
            <div className={`rounded-md border p-3 text-xs ${
              r.riskLevel === "NO_TRADE_WINDOW" ? "border-red-700 bg-red-950/40 text-red-200"
              : r.riskLevel === "HIGH_RISK"     ? "border-orange-700 bg-orange-950/40 text-orange-200"
            ...
            <div className="space-y-1.5 rounded-md border border-slate-800 bg-slate-950/40 p-3">
              <div className="flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-wide text-slate-500">
                ...
                {r.news.highImpactCount > 0 && (
                  <span className="inline-flex items-center rounded-full border border-red-700 px-1.5 py-0.5 text-[9px] normal-case tracking-normal text-red-300">
