...
const STATUS_COLORS: Record<Status, string> = {
  ...
  INVALIDATED: "bg-warning text-white",
  EXECUTED: "bg-primary text-white",
  CANCELED: "bg-secondary text-txt-secondary",
};
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[260px_1fr]">
      ...
      <aside className="space-y-2">
        <button
          ...
          className="w-full rounded-md bg-primary px-3 py-2 text-sm font-semibold text-white hover:bg-primary"
        ...
        <div className="space-y-1">
          {(list.data?.plans ?? []).map((p) => (
            <button
              ...
              className={`w-full rounded-md border px-3 py-2 text-left text-sm ${
                p.id === activeId ? "border-primary bg-secondary" : "border-border bg-card hover:bg-secondary"
              }`}
            >
              <div className="flex items-center justify-between">
                <span className="font-medium text-foreground">{p.symbol ?? "Untitled"}</span>
          ...
          {!list.data?.plans?.length && <p className="text-xs text-txt-muted">No plans yet.</p>}
        </div>
      </aside>

      {/* editor */}
      {!active ? (
        <div className="rounded-lg border border-border bg-muted/40 p-6 text-sm text-txt-secondary">
          Create a new trade plan to begin.
        </div>
      ) : (
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-foreground">Plan #{active.id}</h2>
          ...
          <div className="grid grid-cols-1 gap-3 rounded-lg border border-border bg-muted/40 p-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 flex flex-wrap gap-2 pt-2">
              <button onClick={() => save.mutate(form)} disabled={!editable || save.isPending} className="rounded-md bg-muted px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">
                {save.isPending ? "Saving…" : "Save changes"}
              </button>
              <button onClick={() => validate.mutate()} disabled={!editable || validate.isPending} className="rounded-md bg-primary px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">
                {validate.isPending ? "Validating…" : "Validate plan"}
              </button>
              <button onClick={() => cancel.mutate()} disabled={!editable || cancel.isPending} className="rounded-md border border-danger/40 bg-danger/15 px-3 py-2 text-sm font-semibold text-danger disabled:opacity-50">
                Cancel plan
              </button>
            </div>
          </div>
...
const inputCls = "w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm text-foreground disabled:opacity-60";

function Field({ label, children, full }: { label: string; children: React.ReactNode; full?: boolean }) {
  return (
    <label className={`block text-xs ${full ? "sm:col-span-2" : ""}`}>
      <span className="mb-1 block text-txt-secondary">{label}</span>
      {children}
    </label>
  );
}
