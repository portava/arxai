...
export function PropRuleViolationFeed({ violations }: { violations: V[] }) {
  return (
    <div className="rounded-lg border border-border bg-muted/40 p-4">
      <h3 className="mb-2 text-sm font-semibold text-foreground">Rule violations & warnings ({violations.length})</h3>
      {violations.length === 0
        ? <p className="text-xs text-txt-muted">No violations or warnings yet.</p>
