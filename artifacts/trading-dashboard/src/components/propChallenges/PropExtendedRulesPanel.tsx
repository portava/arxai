  ...
  return (
    <div className="rounded-lg border border-border bg-muted/40 p-4">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-foreground">Extended prop rules</h3>
        ...
        <button
          ...
          className="rounded bg-success/15 px-3 py-1 text-xs font-semibold text-white hover:bg-success disabled:opacity-40"
        >
      ...
      <div className="grid grid-cols-1 gap-3 text-xs sm:grid-cols-2">
        ...
        <div className="rounded border border-border p-3">
          <label className="flex items-center gap-2">
            ...
            <span className="font-medium text-foreground">Trailing drawdown</span>
          </label>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <label className="text-txt-secondary">Amount (0..1)
              <input type="number" step="0.005" min="0.001" max="1" value={draft.trailingDrawdownAmount}
                ...
                className="w-full rounded border border-border bg-background px-2 py-1 font-mono text-foreground" />
            </label>
            <label className="text-txt-secondary">Type
        ...
        {/* Per-trade limits */}
        <div className="rounded border border-border p-3">
          <div className="font-medium text-foreground">Per-trade limits</div>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <label className="text-txt-secondary">Max risk per trade (0..1)
              <input type="number" step="0.005" min="0.001" max="1" value={draft.maxRiskPerTrade}
                onChange={num("maxRiskPerTrade")} disabled={disabled}
                className="w-full rounded border border-border bg-background px-2 py-1 font-mono text-foreground" />
            </label>
            <label className="text-txt-secondary">Max position size (lots)
        ...
        {/* Concurrency */}
        <div className="rounded border border-border p-3">
          <div className="font-medium text-foreground">Concurrency</div>
          <div className="mt-2 grid grid-cols-2 gap-2">
            <label className="text-txt-secondary">Max open trades
              <input type="number" step="1" min="1" max="100" value={draft.maxOpenTrades}
                onChange={num("maxOpenTrades")} disabled={disabled}
                className="w-full rounded border border-border bg-background px-2 py-1 font-mono text-foreground" />
            </label>
            <label className="text-txt-secondary" title="No PENDING status in demo schema — evaluates as INSUFFICIENT_DATA today.">
              Max pending orders
        ...
        {/* Restrictions */}
        <div className="rounded border border-border p-3">
          <div className="font-medium text-foreground">Holding restrictions</div>
          <div className="mt-2 space-y-1">
            <label className="flex items-center gap-2 text-txt-secondary">
              ...
              <span>News trading allowed
                <span className="ml-1 text-[10px] text-warning">(no news provider — INSUFFICIENT_DATA)</span>
              </span>
            </label>
            <label className="flex items-center gap-2 text-txt-secondary">
        ...
        <div className="rounded border border-border p-3 sm:col-span-2">
          <label className="flex items-center gap-2">
            ...
            <span className="font-medium text-foreground">Strict guardrails</span>
          </label>
          <p className="mt-1 text-[11px] text-txt-secondary">
            When enabled, HARD rule violations escalate demo rule status to <span className="font-mono text-warning">BLOCKED</span>{" "}
      ...
      {save.isError && (
        <p className="mt-2 text-[11px] text-danger">{(save.error as Error).message}</p>
      )}
    </div>
  );
}
